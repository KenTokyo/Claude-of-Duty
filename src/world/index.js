...
export class WorldSystem {
  ...
  async init(ctx) {
    ...
    this._practicalLightBudget = Math.max(0, ctx.config.q.practicalLightBudget ?? 42);
    this._lightSlots = Math.max(0, ctx.config.q.pointLightSlots ?? LIGHT_SLOTS);
    this._addLights(A);
  ...
  _addLights(A) {
    ...
    let remaining = this._practicalLightBudget;
    const bulbCount = Math.min(20, A.interiorLights.length, remaining);
    for (const b of A.interiorLights.slice(0, bulbCount)) {
    ...
    remaining -= bulbCount;
    for (const p of A.lampAnchors.slice(0, remaining)) {
   ...
   */
  async prewarmMaterials(ctx = this.ctx, { forward = true } = {}) {
    ...
    try {
      // 1. Forward lit pass. The global orchestrator already compiled the whole
      // scene once and disables this duplicate walk with forward:false.
      if (forward) await this._compile(renderer, scene, camera);
      // 2. Shadow cascades and (when enabled) 3. depth/normal/velocity prepass.
      const overrides = [render.csm?.depthMaterial];
      if (render.needsPrepass) overrides.push(render.gbuffer?.material);
