...
export class SoldierMaterials {
  ...
  get(setName, opts = {}) {
    ...
    m.name = `ai_${setName}`;
    if (!this.simple) this._attachShader(m, d && this.details[d.set] ? d : null, opts.rim);
    else m.userData.owSimpleMaterial = true;
  ...
  glass(tint = [0.06, 0.07, 0.08]) {
    ...
    if (!this.simple) this._attachShader(m, null, 0.5);
    else m.userData.owSimpleMaterial = true;
