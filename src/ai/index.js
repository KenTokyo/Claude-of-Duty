...
export class AiSystem {
  ...
  async init(ctx) {
    ...
    this.materials = new SoldierMaterials(this.rng.fork(), {
      ...
      simple: ctx.config.q.simpleMaterials === true,
    });
    ...
    this._bootNav(ctx);
    // Exhaustive character compilation was a major part of the multi-minute
    // ANGLE/D3D11 boot. The global ?prewarm=1 orchestrator invokes this hook
    // after every subsystem exists and with the correct HDR target bound;
    // normal play lets the smaller low programs compile when first visible.
  }
