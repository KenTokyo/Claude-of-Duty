...
export class SkySystem {
  ...
  async init(ctx) {
    ...
    const envWidth = q.simpleMaterials ? 256 : 512;
    this.envEquirect = hdrTarget(envWidth, envWidth >> 1, { name: 'sky-equirect' });
    ...
    this._forwardFog = q.fogPost === false;
    if (this._forwardFog) {
      // Operation Ironhold's important performance choice: fog participates in
      // the ordinary forward material pass instead of forcing a depth prepass
      // plus another full-screen composite. Stock and extended three materials
      // already contain the fog chunks; the sky shader itself remains untouched.
      this._forwardFogObject = new THREE.FogExp2(0xa39b8d, 0.0034);
      ctx.scene.fog = this._forwardFogObject;
    ...
    console.info(
      `[sky] atmosphere ready · lat ${this.celestial.site.latitudeDeg} · ` +
        `vol ${this._forwardFog ? 'forward fog' : q.volumetrics ? steps + ' steps @1/2' : 'analytic'} · ` +
        `1 unit = ${SCENE_LUX} lx`
    );
  }
  ...
  _applyFog() {
    const f = this._fog;
    if (this._forwardFogObject) {
      // Match the configured extinction approximately over the 120 m play area.
      this._forwardFogObject.density = 0.0034 * Math.sqrt(Math.max(0.05, f.extinction / 1.45e-3));
    }
  ...
  dispose() {
    ...
    if (this.ctx.scene.fog === this._forwardFogObject) this.ctx.scene.fog = null;
