...
export class Engine {
  ...
  async init({ onProgress = null, yieldBetween = false } = {}) {
    const order = this.registry.resolve();
    const total = order.length;
    for (let index = 0; index < total; index++) {
      const sys = order[index];
      const id = sys.constructor.id;
      onProgress?.({ id, index, total, phase: 'start', progress: index / total });
      // Yield after publishing the next stage, not after it finished, so the
      // browser can actually paint "Weltgeometrie …" before that heavy stage.
      if (yieldBetween) await new Promise((resolve) => setTimeout(resolve, 0));
      const t0 = performance.now();
      await sys.init?.(this.ctx);
      const ms = performance.now() - t0;
      if (ms > 50) console.info(`[engine] ${id} init ${ms.toFixed(0)}ms`);
      onProgress?.({ id, index, total, phase: 'done', ms, progress: (index + 1) / total });
