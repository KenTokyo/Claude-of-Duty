...
/**
 * `WebGLRenderer.compile[Async]()` traverses every visible scene object without
 * frustum culling; camera position is not a shader key. Four old "warm poses"
 * therefore awaited the same full scene four times. Keep one representative
 * camera only—the depth passes have their own explicit hooks below.
 */
const WARM_POSES = [
...
export async function prewarm(engine, { onProgress = () => {}, transients = false, drawFrames = false } = {}) {
  ...
  try {
    ...
    // Pass 1: one full visible-scene traversal. Explicit hooks below cover the
    // depth/shadow variants; repeated camera poses cannot create new programs.
    ...
    const hookResults = {};
    // Keep the HDR target bound across every hook. Otherwise three compiles an
    // sRGB canvas variant here and recompiles the linear-HDR variant in frame 1.
    renderer.setRenderTarget(scratchRt);
    for (const sys of hooks) {
      const id = sys.constructor?.id ?? '?';
      try {
        if (sys === renderSys) {
          hookResults[id] = (await sys.prewarmMaterials({
            post: true,
            forward: false,
            shadow: RENDER_SHADOW_WARM,
          })) ?? { ok: true };
        } else if (id === 'world') {
          hookResults[id] = (await sys.prewarmMaterials(engine.ctx, { forward: false })) ?? { ok: true };
        } else {
