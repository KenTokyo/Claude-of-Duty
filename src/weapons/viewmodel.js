...
// Only these weapon materials have passed the complete dynamic pixel,
// transform, visibility and disposal gate. Keep the diagnostic opt-in narrow;
// `viewBatchWeaponMaterial=all` still restores the broad stress-test path.
const DEFAULT_WEAPON_BATCH_MATERIALS = Object.freeze(['cavity', 'rubber', 'brass']);

/** Attribute/index contract used to keep BatchedMesh inputs compatible. */
function batchGeometryKey(geo) {
  const attrs = Object.keys(geo.attributes).sort();
  return `${geo.getIndex() ? 'i' : 'n'}|${attrs
    .map((name) => {
...
/**
 * Dynamic heterogeneous multi-draw for a rigid hierarchy.
 *
 * Source meshes stay in the graph as transform bones but are hidden from the
 * renderer. Their final matrices are copied into a BatchedMesh texture, so all
 * meshes sharing one material become one GPU submission while fingers, bolt,
 * trigger, slide and magazine remain independently animated and hideable.
 */
class DynamicBatchSet {
  constructor(root, meshes, name) {
    this.root = root;
    this.name = name;
    this.batches = [];
    this.sourceMeshes = 0;
    ...
    for (let i = 0; i < meshes.length; i++) {
      ...
      const key = `${mesh.material.uuid}|${mesh.renderOrder}|${batchGeometryKey(mesh.geometry)}`;
    ...
    for (const list of groups.values()) {
      // One source would merely trade a uniform matrix for a texture lookup.
      ...
      let batch = null;
      try {
        batch = new THREE.BatchedMesh(list.length, vertices, indices || vertices, list[0].material);
        batch.name = `${name}-batch-${serial++}`;
        // Keep source insertion order; internal sorting cannot interleave with
        // unbatched meshes and is evaluated separately by the pixel gate.
        batch.sortObjects = false;
        batch.perObjectFrustumCulled = false;
        batch.frustumCulled = false; // same contract as every source view mesh
        batch.castShadow = list.some((m) => m.castShadow);
        batch.receiveShadow = list.some((m) => m.receiveShadow);
        batch.renderOrder = list[0].renderOrder;
        batch.matrixAutoUpdate = false;
        batch.updateMatrix();

        for (const [g] of unique) unique.set(g, batch.addGeometry(g));
        const entries = [];
        for (let i = 0; i < list.length; i++) {
          const source = list[i];
          const instanceId = batch.addInstance(unique.get(source.geometry));
          entries.push({
            source,
            instanceId,
        ...
        root.add(batch);
        for (let i = 0; i < entries.length; i++) entries[i].source.visible = false;
        this.batches.push({ batch, entries });
        this.sourceMeshes += entries.length;
        this.draws++;
      } catch (err) {
        batch?.dispose();
        console.warn(`[weapons] skipped ${name} batch: ${err?.message ?? err}`);
      }
    }
  ...
  /** Copy root-local transforms and parent visibility without allocating. */
  sync() {
    if (this.batches.length === 0) return;
    this.root.updateWorldMatrix(true, true);
    _batchInv.copy(this.root.matrixWorld).invert();
    for (let i = 0; i < this.batches.length; i++) {
      const { batch, entries } = this.batches[i];
      for (let j = 0; j < entries.length; j++) {
        const e = entries[j];
        const source = e.source;
        _batchMatrix.multiplyMatrices(_batchInv, source.matrixWorld);
        batch.setMatrixAt(e.instanceId, _batchMatrix);

        let visible = e.visible;
        for (let p = source.parent; p && p !== this.root; p = p.parent) {
          if (visible) visible = p.visible;
        }
        if (e.batchVisible !== visible) {
          batch.setVisibleAt(e.instanceId, visible);
          e.batchVisible = visible;
        }
      }
    }
  }
  ...
  dispose() {
    for (let i = 0; i < this.batches.length; i++) {
      const batch = this.batches[i].batch;
...
export class Viewmodel {
  ...
  /**
   * Collapse rigid viewmodel pieces by material after every weapon has finished
   * its contact-mask bake. Called once by WeaponSystem after model construction.
   */
  finalizeBatches() {
    if (this._batchesFinalized) return this.batchStats;
    this._batchesFinalized = true;

    const gl = this.ctx.peek('render')?.renderer?.getContext?.();
    const params = new URLSearchParams(globalThis.location?.search ?? '');
    // BatchedMesh saves only a handful of view draws and measured slower in the
    // production scene, so preserve it as an explicit diagnostic path only.
    const requested = params.get('viewBatch') === '1';
    const multiDraw = requested && !!gl?.getExtension?.('WEBGL_multi_draw');
    const armsEnabled = multiDraw && params.get('viewBatchArms') !== '0';
    const weaponsEnabled = multiDraw && params.get('viewBatchWeapons') !== '0';
    // The diagnostic opt-in is deliberately limited to configurations that pass
    // every dynamic state bit-for-bit: identity-space glove meshes, no sleeves,
    // and an explicit weapon-material whitelist. URL values remain narrower or
    // broader overrides (`viewBatchWeaponMaterial=all` restores the stress path).
    const gloveMode = armsEnabled ? (params.get('viewBatchGloves') ?? 'identity') : '0';
    const sleeveParam = params.get('viewBatchSleeves');
    const sleevesEnabled = armsEnabled && sleeveParam !== null && sleeveParam !== '0';
    const fixedEnabled = weaponsEnabled && params.get('viewBatchWeaponFixed') !== '0';
    const movingEnabled = weaponsEnabled && params.get('viewBatchWeaponMoving') !== '0';
    const weaponMaterialParam = params.get('viewBatchWeaponMaterial');
    const weaponMaterials = weaponMaterialParam === 'all' || weaponMaterialParam === '*'
      ? null
      : (weaponMaterialParam === null
          ? [...DEFAULT_WEAPON_BATCH_MATERIALS]
          : weaponMaterialParam.split(',').map((value) => value.trim()).filter(Boolean));
    const arms = {
      enabled: false,
      sourceMeshes: 0,
      draws: 0,
      saved: 0,
      gloves: { enabled: gloveMode !== '0', sourceMeshes: 0, draws: 0, saved: 0 },
      sleeves: { enabled: sleevesEnabled, sourceMeshes: 0, draws: 0, saved: 0 },
    };
    const weapons = {
      enabled: false,
      sourceMeshes: 0,
      ...
      saved: 0,
      fixed: fixedEnabled,
      moving: movingEnabled,
      materials: weaponMaterials ?? 'all',
    };

    if (gloveMode !== '0' || sleevesEnabled) {
      const options = { gloveMode, sleeves: sleevesEnabled };
      const right = this._finalizeArmBatches(this.armR, 'right', options);
      const left = this._finalizeArmBatches(this.armL, 'left', options);
      arms.gloves.sourceMeshes = right.gloves.sourceMeshes + left.gloves.sourceMeshes;
      arms.gloves.draws = right.gloves.draws + left.gloves.draws;
      arms.gloves.saved = arms.gloves.sourceMeshes - arms.gloves.draws;
      arms.sleeves.sourceMeshes = right.sleeves.sourceMeshes + left.sleeves.sourceMeshes;
      arms.sleeves.draws = right.sleeves.draws + left.sleeves.draws;
      arms.sleeves.saved = arms.sleeves.sourceMeshes - arms.sleeves.draws;
      arms.sourceMeshes = arms.gloves.sourceMeshes + arms.sleeves.sourceMeshes;
      arms.draws = arms.gloves.draws + arms.sleeves.draws;
      arms.saved = arms.sourceMeshes - arms.draws;
      arms.enabled = arms.sourceMeshes > 0;
    }

    if (fixedEnabled || movingEnabled) {
      for (const w of this.weapons.values()) {
        let meshes = fixedEnabled && movingEnabled
          ? w.meshes
          : w.meshes.filter((mesh) => (mesh.parent === w.group) === fixedEnabled);
        if (weaponMaterials) {
          meshes = meshes.filter((mesh) =>
            weaponMaterials.some((material) => mesh.name.endsWith(`-${material}`))
          );
        }
        w.batchSet = new DynamicBatchSet(w.group, meshes, `weapon-${w.id}`);
        weapons.sourceMeshes += w.batchSet.sourceMeshes;
        weapons.draws += w.batchSet.draws;
      }
      weapons.saved = weapons.sourceMeshes - weapons.draws;
      weapons.enabled = weapons.sourceMeshes > 0;
    }

    const sourceMeshes = arms.sourceMeshes + weapons.sourceMeshes;
    const draws = arms.draws + weapons.draws;
    this.batchStats = {
      enabled: sourceMeshes > 0,
      sourceMeshes,
      draws,
      saved: sourceMeshes - draws,
      arms,
      weapons,
    };
  ...
  _finalizeArmBatches(arm, label, { gloveMode, sleeves }) {
    const gloves = { sourceMeshes: 0, draws: 0 };
    const sleeveStats = { sourceMeshes: 0, draws: 0 };
    if (gloveMode !== '0') {
      const gloveMeshes = [];
      arm.glove.traverse((o) => {
        if (!o.isMesh || o.isBatchedMesh) return;
        if (gloveMode === 'direct' && o.parent !== arm.glove) return;
        if (gloveMode === 'nested' && o.parent === arm.glove) return;
        if (gloveMode === 'identity' || (gloveMode === 'safe' && o.parent === arm.glove)) {
          o.updateMatrix();
          if (o.parent !== arm.glove || !o.matrix.equals(_identityMatrix)) return;
        }
        gloveMeshes.push(o);
      });
      arm._gloveBatchSet = new DynamicBatchSet(arm.glove, gloveMeshes, `arm-${label}-glove`);
      gloves.sourceMeshes = arm._gloveBatchSet.sourceMeshes;
      gloves.draws = arm._gloveBatchSet.draws;
    }
    if (sleeves) {
      arm._sleeveBatchSet = new DynamicBatchSet(
        arm.root,
        [arm.upper, arm.fore],
        `arm-${label}-sleeve`
      );
      sleeveStats.sourceMeshes = arm._sleeveBatchSet.sourceMeshes;
      sleeveStats.draws = arm._sleeveBatchSet.draws;
    }
    return { gloves, sleeves: sleeveStats };
  }

  _syncBatches(w = this.active) {
    this.armR._gloveBatchSet?.sync();
    this.armR._sleeveBatchSet?.sync();
    this.armL._gloveBatchSet?.sync();
    this.armL._sleeveBatchSet?.sync();
    w?.batchSet?.sync();
  }
  ...
  update(dt, s) {
    ...
    // Moving groups have reached their final pose; upload all heterogeneous
    // instance matrices once, immediately before the view scene is rendered.
    this._syncBatches(w);
  ...
  dispose() {
    ...
    this.weapons.clear();
    this.armL._gloveBatchSet?.dispose();
    this.armL._sleeveBatchSet?.dispose();
    this.armR._gloveBatchSet?.dispose();
    this.armR._sleeveBatchSet?.dispose();
    this.armL.dispose();
    this.armR.dispose();
