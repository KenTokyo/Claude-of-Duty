...
// Viewmodel batching only needs the renderer's WebGL context. Mirror the
// production render service here so the standalone harness exercises the same
// WEBGL_multi_draw capability check and fallback path as the game.
const render = { renderer };
const services = { materials, render };
const ctx = {
  scene,
  camera,
  viewScene: scene,
  ...
  get: (id) => services[id] ?? null,
  peek: (id) => services[id] ?? null,
  has: (id) => id in services,
};
...
const batchStats = vm.finalizeBatches();
vm.setActive(WEAPON);
if (!ARMS) {
