...
export class RenderSystem {
  ...
  async init(ctx) {
    ...
    // ---- GPU-paced dynamic resolution -------------------------------------
    // Capture runs stay at an exact, fixed raster size; normal play follows
    // measured GPU time with a slow hysteresis controller. Unlike rAF delta,
    // EXT_disjoint_timer_query_webgl2 excludes vsync waiting and CPU work, so a
    // CPU spike cannot needlessly blur the frame. Browsers that hide the timer
    // transparently fall back to frame pacing.
    const url = new URLSearchParams(location.search);
    const fpsParam = Number(url.get('fps'));
    const targetFps = Math.max(
      30,
      Math.min(240, Number.isFinite(fpsParam) && fpsParam > 0 ? fpsParam : q.targetFps || 60)
    );
    const adaptiveEnabled =
      q.adaptiveResolution !== false &&
      cfg.deterministic !== true &&
      url.get('adaptive') !== '0';
    this.renderScale = q.renderScale;
    // Do not even open timer queries when adaptive scaling is disabled. Apart
    // from being wasted work in deterministic captures, some ANGLE/D3D11
    // drivers reset the GPU if a whole-frame query encloses first-use shader
    // compilation. The controller never consumes those samples, so querying in
    // that mode has no upside.
    this.gpuTimer = adaptiveEnabled ? new GpuFrameTimer(renderer) : null;
    ...
    this._gpuTimerDelayFrames = Math.max(0, q.gpuTimerDelayFrames ?? 0);
    this.performance = {
      adaptive: adaptiveEnabled,
      timer: this.gpuTimer?.supported ? 'gpu' : adaptiveEnabled ? 'frame' : 'off',
      targetFps,
    ...
    // the same cheap arrangement operation-ironhold uses.
    this.directViewmodel =
      q.directViewmodel === true && !this.taa && !this.motionBlur && !this.dof;
    // MSAA on the separate viewmodel target only. The direct path is spatially
    // antialiased by the final FXAA and allocates no extra full-size HDR target.
    this._viewSamples = this.directViewmodel
      ? 0
      : this.qLevel >= 2
        ? 4
        : this.qLevel >= 1
    ...
    // Three directional lights + hemisphere mirrors Ironhold's inexpensive
    // view rig. Rich presets retain the fourth warm bounce light.
    if (q.simpleLighting === true) this.viewBounce.visible = false;
    ...
    // The real SkySystem initializes before any world/view material exists, so
    // constructing a 512x256 CPU equirect + PMREM here only to replace it in the
    // very next subsystem doubled environment-map boot work. Keep the fallback
    // solely for standalone render harnesses that did not register a sky.
    const env = ctx.has('sky')
      ? null
      : buildFallbackEnvironment(renderer, this._dirFromLight(this.sun, this.sunDir));
    this.envTarget = env?.target ?? null;
    this.envEquirect = env?.equirect ?? null;
    this.envMap = this.envTarget?.texture ?? null;
    if (!ctx.scene.environment && this.envMap) ctx.scene.environment = this.envMap;
    if (!ctx.scene.background && this.envEquirect) ctx.scene.background = this.envEquirect;
  ...
  registerPass(pass) {
    ...
    // The low forward path starts with no ping-pong buffers. Allocate them only
    // if a subsystem really registers a post pass.
    if (!this.pingRt[0]) this._resizePingTargets(this.screenSize.width, this.screenSize.height, true);
   ...
   * @param {boolean} [opts.forward=true] compile world/view forward materials
   ...
   */
  async prewarmMaterials({ post = true, forward = true, shadow = this.frame === 0 } = {}) {
    ...
    try {
      ...
      // 2. Forward lit programs. The global orchestrator already compiles these
      // once with the correct HDR target bound, so it can set forward:false and
      // avoid walking/awaiting the same scene again.
      if (forward) {
        try {
  ...
  _collectPassMaterials(out) {
    ...
    add(this.composite);
    if (!this.directViewmodel) add(this.viewComposite);
    add(this.fxaa);
    if (this.gtao) {
      add(this.gtao.core);
  ...
  _resizePingTargets(w, h, needed) {
    if (!needed) {
      this.pingRt[0]?.dispose();
      this.pingRt[1]?.dispose();
      this.pingRt[0] = this.pingRt[1] = null;
      return;
    }
    if (
      this.pingRt[0] &&
      this.pingRt[0].width === w &&
      this.pingRt[0].height === h
    ) return;
    this.pingRt[0]?.dispose();
    this.pingRt[1]?.dispose();
    this.pingRt[0] = hdrTarget(w, h, { name: 'ping0' });
    this.pingRt[1] = hdrTarget(w, h, { name: 'ping1' });
  }

  resize(w, h, ctx, preserveExposure = false) {
    this._cssWidth = Math.max(1, w | 0);
    this._cssHeight = Math.max(1, h | 0);
    const pr = Math.min(globalThis.devicePixelRatio || 1, this.q.maxPixelRatio ?? 1.5);
    this.renderer.setPixelRatio(pr);
    this.renderer.setSize(this._cssWidth, this._cssHeight, false);

    const dw = Math.max(1, Math.floor(this._cssWidth * pr));
    const dh = Math.max(1, Math.floor(this._cssHeight * pr));
    const rw = Math.max(1, Math.floor(dw * this.renderScale));
    const rh = Math.max(1, Math.floor(dh * this.renderScale));
    ...
    this.viewRt?.dispose();
    this.viewRt = this.directViewmodel
      ? null
      : hdrTarget(rw, rh, {
          depthBuffer: true,
    ...
    this._resizePingTargets(
      rw,
      rh,
      !this.directViewmodel || !!this.dof || this.passes.length > 0
    );
    ...
    this.normalTexture = this.needsPrepass ? this.gbuffer.normalTexture : null;

    for (const p of this.passes) p.resize?.(rw, rh);
    this.taa?.reset();
    if (!preserveExposure) {
      this.exposure.reset();
      this._forceExposure = true;
    }
  }
  ...
  render(ctx) {
    ...
    gpuTimer?.poll();
    const gpuReady = !!gpuTimer?.supported && this.frame > this._gpuTimerDelayFrames;
    let scale = 0;
    if (gpuReady) {
    ...
    if (scale > 0 && Math.abs(scale - this.renderScale) > 0.005) {
      this.renderScale = scale;
      // Auto-exposure is a 1x1 luminance history and does not depend on raster
      // size. Preserve it across adaptive resizes or every scale step would
      // cause a visible exposure flash; spatial/temporal buffers still resize
      // and TAA is reset below.
      this.resize(this._cssWidth, this._cssHeight, ctx, true);
    }
    ...
    this._syncSun(camera);
    if (!this.patcher.simple) this._updateRooms();
    ...
    // ---- 9. viewmodel ------------------------------------------------------
    // Rich presets keep the separate target described in the header. The low
    // forward path has no depth-driven world post here, so it clears HDR depth
    // and draws directly, avoiding one target and one full-screen composite.
    ...
    // and still blooms.
    if (this._viewVisible && !this.directViewmodel) {
    ...
    if (this._forceExposure || this.frame === 1 || this.frame % this._exposureInterval === 0) {
      exposureTex = this.exposure.update(
        renderer,
        color,
        this.screenSize.width,
        this.screenSize.height,
        s.autoExposure ? dt * this._exposureInterval : 1e3,
  ...
  _renderDebug(renderer, color) {
    ...
    const map = {
      ao: [this.aoTexture, 0],
      normal: [this.normalTexture, 1],
      velocity: [this.velocityTexture, 2],
      depth: [this.depthTexture, 3],
