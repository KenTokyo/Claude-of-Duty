...
export class MaterialPatcher {
  constructor(csmUniforms, opts) {
    ...
    this.key = `ow-patch-${PATCH_VERSION}-${opts.cascades}-${opts.quality}-${this.simple ? 'simple' : 'full'}`;
  ...
  patch(material) {
    ...
    material.onBeforeCompile = function (shader, renderer) {
      ...
      // The production low path keeps the custom CSM but omits the large
      // AO/SSR/interior-lighting program. It is deliberately close to
      // operation-ironhold's stock forward lighting and shares programs across
      // palette variants instead of compiling one giant permutation per surface.
      const lightInjection = simple
        ? `getDirectionalLightInfo( directionalLight, directLight );
        directLight.color *= receiveShadow ? owSunShadow( directionalLight.direction, geometryPosition, geometryNormal ) : 1.0;`
        : `getDirectionalLightInfo( directionalLight, directLight );
