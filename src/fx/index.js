...
export class FxSystem {
  ...
  async init(ctx) {
    ...
    this.hazeSys.enabled = q.haze !== false;
    this._hazeOff = this.hazeSys.enabled
      ? this.render?.registerPass?.(this.hazeSys.pass) ?? null
      : null;
  ...
  _attachView() {
    ...
    if (!this.viewLights) {
      this.viewLights = new LightPool(
        this.ctx.viewScene,
        this.ctx.config.q.viewPointLightSlots ?? 2
      );
  ...
  prewarmMaterials() {
    ...
    try {
      ...
      if (this.hazeSys.enabled) this.hazeSys.prewarm(renderer);
    } finally {
