...
export class MaterialSystem {
  ...
  async init(ctx) {
    ...
    this._quality =
      ctx?.config?.quality === 'low' ? 0.25 : ctx?.config?.quality === 'medium' ? 0.75 : 1;
    this._tryBuild();
  }
  ...
  _tryBuild() {
    ...
    if (this._simple) {
      // The forward performance path uses each surface's ordinary albedo/normal/
      // ORM set and stock PBR shaders. Do not build two shared maps that no
      // material will sample.
    ...
    if (ms > 30) {
      console.info(`[materials] ${this._simple ? 'simple forge' : 'shared maps'} ${ms.toFixed(0)}ms`);
    }
    return true;
  }
  ...
  get(name, opts = {}) {
    ...
    const mat = new Ctor({
      // The extended shader applies tint itself. The simple shader is stock
      // three.js, so its material colour carries the same texture multiply.
      color: this._simple ? p.tint : 0xffffff,
      roughness: this._simple ? Math.max(0.04, Math.min(1, p.roughness?.[0] ?? 1)) : 1,
      metalness: 1,
      dithering: true,
    });
    mat.name = matKey;
    ...
    if (set) {
      ...
      mat.normalScale.set(
        this._simple ? p.normalStrength ?? 1 : 1,
        this._simple ? p.normalStrength ?? 1 : 1
      );
      mat.roughnessMap = set.orm;
      if (this._simple) {
        // TextureForge packs roughness in G and metalness in B, matching the
        // channels three's stock materials consume. This collapses dozens of
        // giant procedural shader variants onto a few shared PBR programs.
        mat.metalnessMap = set.orm;
        mat.userData.owSimpleMaterial = true;
      }
    ...
    // World vertex colours are wear/grime masks, not diffuse RGB. Only the
    // extended shader knows that contract; stock PBR must leave them alone.
    if (p.vertexMasks && !this._simple) mat.vertexColors = true;
    applyProps(mat, threeProps);

    if (set && this._simple && mat.transparent && !p.alphaMask) {
      // Surface albedo stores procedural height in A. The full shader consumes
      // it as height and intentionally leaves opacity alone; stock three would
      // accidentally multiply transparent glass by that height field.
      mat.customProgramCacheKey = () => 'ow-simple-ignore-map-alpha';
    ...
    if (set && !this._simple) extendMaterial(mat, p, this._shared);
